...
class InspectionSheet(models.Model):
    ...
    @api.depends('picking_id','production_id','production_id.product_qty','picking_id.move_lines.product_uom_qty')
    ...
    def process_quantities(self):
        if self.picking_id:
            ...
            if self.quantity_accepted or self.under_deviation:
                ...
                lot_qty_done=self.quantity_accepted+self.under_deviation
                # self.picking_id.move_line_nosuggest_ids = [(0,0,line)]
            # if self.picking_id.picking_type_id.code == 'incoming' and lot_qty_done > 0:
            #     if self.product_id.custom_lot == False:
            #         self.lot_id=self.env['stock.production.lot'].create({'name':"""{}/{}/{}/{}""".format(self.picking_id.partner_id.name[:4],self.picking_id.scheduled_date.date(),self.picking_id.name.split('/')[-1],self.env['ir.sequence'].next_by_code('stock.lot.serial')),
            #                 'product_id':self.product_id.id,'company_id':self.company_id.id}).id
            #         line.update({'lot_name':self.lot_id.name})
            #         self.picking_id.move_line_nosuggest_ids = [(0,0,line)]
    ...
    def revise(self):
        ...
        self.revised_sheet_ids.quality_check_ids.quality_state = 'pass'
        ...
        self.state = 'open'
        self.quality_check_ids.quality_state = 'none'
